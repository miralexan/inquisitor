...
object Actions {
  ...
  object Create extends Action with StrictLogging {
    ...
    override def apply(config: Config): Unit = {
      ...
      val db = Database.forURL(new URI("jdbc:sqlite", config.outFile.toURI().getSchemeSpecificPart, null).toString(), driver = "org.sqlite.JDBC")
      val databaseExecution = db.run(setupAction >> insertTestResultsForTestCases(testResultStream))
      databaseExecution.onComplete(_ => db.close())
      Await.result(databaseExecution, 30 seconds).fold(logger.info("No results could be added."))(x => logger.info(s"Added $x test results"))
    }
  ...
  object Add extends Action with StrictLogging {
    import scala.concurrent.ExecutionContext.Implicits.global
    ...
    override def apply(config: Config): Unit = {
      ...
      val db = Database.forURL(new URI("jdbc:sqlite", config.outFile.toURI().getSchemeSpecificPart, null).toString(), driver = "org.sqlite.JDBC")
      val databaseExecution = db.run(insertTestResultsForTestCases(testResultStream))
      databaseExecution.onComplete(_ => db.close())
      Await.result(databaseExecution, 30 seconds).fold(logger.info("No results could be added."))(x => logger.info(s"Added $x test results"))
    }
  ...
  object Validate extends Action with StrictLogging {
    import scala.concurrent.ExecutionContext.Implicits.global

    override def apply(config: Config) = {
      val db = Database.forURL(new URI("jdbc:sqlite", config.outFile.toURI().getSchemeSpecificPart, null).toString(), driver = "org.sqlite.JDBC")
      val f = db.run(getTestResultsInStatus()).map(tcs => if (tcs.isEmpty) logger.info("No unaccounted errors!") else tcs.foreach(tc => logger.warn(s"No successes recorded for ${tc.className}#${tc.name}")))
